...
export function ParkingSpaces({ parkingLotId }: ParkingSpacesProps) {
    ...
    const createMutation = useMutation({
        mutationFn: async (data: { space_number: string; status: string }) => {
            const response = await fetch(`/api/admin/parking-lots/${parkingLotId}/spaces`, {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) throw new Error('Failed to create parking space')
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: { space_number: string; status: string } }) => {
            const response = await fetch(`/api/admin/parking-lots/${parkingLotId}/spaces/${id}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
            ...
            if (!response.ok) throw new Error('Failed to update parking space')
            return response.json()
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['parkingSpaces', parkingLotId] })
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            ...
            if (!response.ok) throw new Error('Failed to delete parking space')
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['parkingSpaces', parkingLotId] })
